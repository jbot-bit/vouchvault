import { pgTable, text, integer, timestamp, boolean, bigint, index, unique, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const vouchEntries = pgTable('vouch_entries', {
  id: integer('id').primaryKey().generatedAlwaysAsIdentity(),
  ...
  targetProfileId: integer('target_profile_id').notNull().references(() => businessProfiles.id),
  ...
  return {
    reviewerTargetCreatedIdx: index('vouch_entries_reviewer_target_created_idx').on(table.reviewerTelegramId, table.targetUsername, table.createdAt),
    targetStatusCreatedIdx: index('vouch_entries_target_status_created_idx').on(table.targetUsername, table.status, table.createdAt),
    statusCreatedIdx: index('vouch_entries_status_created_idx').on(table.status, table.createdAt),
    legacySourceUniqueIdx: uniqueIndex('vouch_entries_legacy_source_unique').on(table.legacySourceChatId, table.legacySourceMessageId),
  };
});
