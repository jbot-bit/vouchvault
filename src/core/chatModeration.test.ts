...
test("findHits: member-typed vouches do NOT match (v9: vouches are load-bearing group content)", () => {
  // v9 deleted the prior vouch_* regex patterns. Members posting
  // "Pos vouch @x cash correct" / "Neg vouch @y robbed me" / "+vouch @z"
  // is exactly what the welcome message asks for; moderating them was
  // a v6-era leftover that contradicted v9.
  for (const text of [
    "POS Vouch @bobbiz",
    "neg vouch @x",
    "Mix Vouch for someone",
    "I vouch for @some_user, solid bloke",
    "vouching for @x",
    "vouched for @y last week",
    "+vouch @x",
    "-vouch @y for being a scammer",
    "the vouch system is great",
    "I gave a vouch yesterday",
  ]) {
    const r = findHits(text);
    assert.equal(r.matched, false, `should NOT match: "${text}"`);
  }
});
